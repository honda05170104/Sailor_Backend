...
const DEFAULT_PROMOTION_COUPONS = [
  {
...
export default {
  name: '025_init_promotions',
  async up(db) {
    const now = new Date();

    await db.collection('promotions').createIndex({ key: 1 }, { unique: true });
    await db.collection('promotions').createIndex({ type: 1 });
    await db.collection('promotions').createIndex({ enabled: 1 });
    await db
      .collection('promotionClaims')
      .createIndex({ user: 1, claimKey: 1 }, { unique: true });
    await db.collection('promotionClaims').createIndex({ promotion: 1 });

    for (const seed of DEFAULT_PROMOTION_COUPONS) {
    ...
    for (const seed of DEFAULT_PROMOTIONS) {
